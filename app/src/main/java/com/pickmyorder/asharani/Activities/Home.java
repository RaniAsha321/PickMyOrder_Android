...
import com.pickmyorder.asharani.Adapters.Adapter_Cart_Menu;
import com.pickmyorder.asharani.Adapters.Adapter_Select_City;
import com.pickmyorder.asharani.Adapters.Banner_Adapter;
import com.pickmyorder.asharani.ApiLogin_Interface;
import com.pickmyorder.asharani.BuildConfig;
import com.pickmyorder.asharani.Fragments.Fragment_Brands;
...
import com.pickmyorder.asharani.Fragments.AccountDetails;
import com.pickmyorder.asharani.Fragments.Cart_Menu;
import com.pickmyorder.asharani.Fragments.Dynamic_Catalogous;
import com.pickmyorder.asharani.Fragments.EmptyCart;
import com.pickmyorder.asharani.Fragments.Home_Categories;
import com.pickmyorder.asharani.Fragments.Orders_Menu;
import com.pickmyorder.asharani.Fragments.Projects;
import com.pickmyorder.asharani.Fragments.Quotes_Menu;
import com.pickmyorder.asharani.Fragments.Search_items;
import com.pickmyorder.asharani.Fragments.Wholeseller;
...
import com.pickmyorder.asharani.MyInterface;
import com.pickmyorder.asharani.Nav_Drawer;
...
public class Home extends AppCompatActivity
    ...
    static List<BannersArray> myAdsList;
    List<BannersArray> myAdsArrayList;
    ...
    public static RelativeLayout layout_banner;
...
/*
        ...
        String video_stopped = Paper.book().read("video_stopped");

        if(video_stopped != null && video_stopped.equals("1")){
...
*/
    ...
    /************************************************ SET PERMISSIONS ON NAVIGATION BUTTON CLICK****************************************************************************************/
        ...
        if(Paper.book().read("permission_brands", "5").equals("1")){

            nav_Menu.findItem(R.id.nav_brands).setVisible(true);

        }

        else if(Paper.book().read("permission_brands", "5").equals("0")){

            nav_Menu.findItem(R.id.nav_brands).setVisible(false);
        }
        ...
        idlayout.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if  ((Search_product_layout.getVisibility() == View.GONE) &&(Search_postcode_layout.getVisibility() == View.VISIBLE)) {
                    ...
                    else if(search_txtvw.getHint().toString().equals("Search Brands")){
    ...
    public static void BannerCall() {

        Paper.book().write("video_stopped","0");
        ...
        timer1.schedule(timerTask1, time, time);

    }

    private void getAds(String status) {
        ...
        service.BANNER_CALL(status).enqueue(new Callback<ModelBanner>() {
            @Override
            public void onResponse(Call<ModelBanner> call, Response<ModelBanner> response) {

                assert response.body() != null;

                Log.e("ads", response.body().getStatusCode()+"");
                if (response.body().getStatusCode().equals(200)) {
                   ...
                   // timer.cancel();
                    ...
                    myAdsList.clear();
                     ...
                     myAdsArrayList = response.body().getBannersArray();

                    Log.e("ads2", myAdsArrayList.size()+"");

                    for (int i = 0; i < myAdsArrayList.size(); i++) {

                         BannersArray bannersArray = new BannersArray();
                         bannersArray.setSource(response.body().getBannersArray().get(i).getSource());
                         bannersArray.setUrl(response.body().getBannersArray().get(i).getUrl());
                         bannersArray.setType(response.body().getBannersArray().get(i).getType());

                         myAdsList.add(bannersArray);
                     }

                     adapter = new Banner_Adapter(Home.this,myAdsList,Integer.parseInt(response.body().getCarouseltime()));

                     viewPager.setAdapter(adapter);

                     if (response.body().getPremissioncarousel().equals("1")){
                     ...
                     time = Integer.parseInt(response.body().getCarouseltime());
                     time = time*1000;

                     Log.e("timer_speed",time+"");

                     Paper.book().write("timer_time",time+"");
            ...
            @Override
            public void onFailure(Call<ModelBanner> call, Throwable t) {
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        switch(id){
            ...
            case R.id.nav_van_stock:
                ...
                Paper.book().write("tab_type","2");
                Paper.book().write("menu_Cat","");
                Paper.book().write("whole_seller","2");
                Paper.book().write("save_business","");
            ...
            case R.id.nav_brands:
                ...
                Paper.book().write("pop_cancel_up","1");
                Paper.book().write("pay_Card_status","0");
                Paper.book().write("search","2");
               // getProducts(Paper.book().read("unique_id"),"0");

                fragmentTransaction=getSupportFragmentManager().beginTransaction().addToBackStack("category").replace(R.id.containerr,new Fragment_Brands());
                fragmentTransaction.commit();
                ...
                Search_postcode_layout.setVisibility(View.GONE);
                search_txtvw.setText("");
               // search_txtvw.setHint("Search Brands");
            ...
            case R.id.nav_wholesellers:
                ...
                Search_product_layout.setVisibility(View.GONE);
                Search_postcode_layout.setVisibility(View.VISIBLE);
                Paper.book().write("tab_type","4");
                //String city = Paper.book().read("city");
                Paper.book().write("menu_Van_Stock","");
                Paper.book().write("menu_Brand","");
                Paper.book().write("menu_Wholesaler","1");
                search_postcode.setText(sharedPreferences.getString("city", ""));
    ...
    public void getWholesaler_Category(String id, String isVan) {
        ...
        service.WHOLESALER_CATEGORY(id,isVan).enqueue(new Callback<ModelProductsCategory>() {
            @Override
    ...
    public void getBrandsCategory(String id){
        ...
        service.Brand_Category_Call(id).enqueue(new Callback<ModelProductsCategory>() {
            @Override
            public void onResponse(Call<ModelProductsCategory> call, Response<ModelProductsCategory> response) {
                ...
                if (response.body().getStatusCode().equals(200)) {
                    ...
                    if (response.body().getNumberofawating() != null){
                    ...
                    fragmentTransaction=getSupportFragmentManager().beginTransaction().addToBackStack("brands_category").replace(R.id.containerr,new Home_Categories());
                    fragmentTransaction.commit();
